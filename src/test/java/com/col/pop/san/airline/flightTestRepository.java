...
public class flightTestRepository {
    ...
    @Test
    public void getsetIdTest2() {
        final Integer flightId = 1;
        final Integer airplaneId = 1;

        // Create available seats using a stream for conciseness
        List<SeatAvailable> availableSeats = Stream.of(

                new SeatAvailable(1, "A", 1, 1, 1),
                new SeatAvailable(2, "A", 2, 1, 1),
                new SeatAvailable(3, "A", 3, 1, 1),
                new SeatAvailable(4, "A", 4, 1, 1),
                new SeatAvailable(29, "B", 1, 1, 1),
                new SeatAvailable(30, "B", 2, 1, 1),
                new SeatAvailable(31, "B", 3, 1, 1),
                new SeatAvailable(32, "B", 4, 1, 1),
                new SeatAvailable(105, "F", 1, 1, 1),
                new SeatAvailable(106, "F", 2, 1, 1),
                new SeatAvailable(107, "F", 3, 1, 1),
                new SeatAvailable(108, "F", 4, 1, 1),
                new SeatAvailable(133, "G", 1, 1, 1),
                new SeatAvailable(134, "G", 2, 1, 1),
                new SeatAvailable(135, "G", 3, 1, 1),
                new SeatAvailable(136, "G", 4, 1, 1)

                ).collect(Collectors.toList());

        List<PassengerResponse> listFinal = checkinservice.getPassengerAttributesByFlightId(flightId, availableSeats, airplaneId);
        System.out.println("listFinal = " + listFinal);
    }
